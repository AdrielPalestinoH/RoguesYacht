...
export default function Inventory() {
  ...
  const openCreateModal = () => {
    setEditingId(null);
    setFormData({ name: '', category: 'bebida', price_mxn: '', description: '' });
    setIsModalOpen(true);
  };
              ...
              <tr key={item.id} className="hover:bg-slate-50/50 transition">
                ...
                <td className="p-5 text-right space-x-3">
                  <button 
                    ...
                    className="text-[#4ec6c6] hover:text-[#1a2e4a] font-bold text-sm transition"
                  >
                    Editar
                  </button>
                  <button 
                    onClick={() => deleteItem(item.id)}
                    className="text-red-300 hover:text-red-600 font-bold text-sm transition"
                  >
                    Eliminar
                  </button>
                </td>
        ...
        <div className="fixed inset-0 z-[1000] flex items-center justify-center p-6">
          ...
          <div className="relative bg-white w-full max-w-md rounded-[40px] p-10 shadow-2xl">
            <h2 className="text-2xl font-serif mb-6 text-[#1a2e4a]">
              {editingId ? 'Editar Producto' : 'Nuevo Add-on'}
            </h2>
            <form onSubmit={handleSave} className="space-y-4">
              ...
              <div className="flex gap-4 pt-4">
                ...
                <button 
                  type="submit" 
                  disabled={isSaving}
                  className="flex-[2] py-4 bg-[#1a2e4a] text-white font-bold rounded-2xl hover:bg-[#4ec6c6] transition disabled:bg-slate-300"
                >
